...
func RegisterCodeToolWithRuntime(s *server.MCPServer, getStore func() *storage.Store, getCodeRuntime func() CodeRuntime) {
	...
	s.AddTool(
		mcp.NewTool("code",
			...
			mcp.WithNumber("line",
				mcp.Description("0-based line of the symbol to rename (rename). symbols and find report 1-based lines, so subtract 1."),
			),
			mcp.WithNumber("character",
				mcp.Description("0-based character offset on that line (rename)."),
			),
			mcp.WithString("newName",
				mcp.Description("New symbol name (rename)."),
			),
			mcp.WithNumber("depth",
				mcp.Description("Nest child symbols to this depth (symbols)."),
			),
			mcp.WithBoolean("include_body",
				mcp.Description("Include each symbol's source body (symbols, find)."),
			),
			mcp.WithBoolean("force",
				mcp.Description("Delete without checking for external references (delete)."),
			),
		),
