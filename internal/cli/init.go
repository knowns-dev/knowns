...
func renderCompatibilityInstructionContent(relativePath, platform, projectRoot string) string {
	...
	sb.WriteString("- A Memory is a fact the NEXT session needs, written from an outcome you reached. Never from a phrase in a request: a prompt is a request, not a conclusion. If an entry carries nothing beyond the message that prompted it, it is not a Memory.\n")
	sb.WriteString("- `pattern`, `convention` and `failure` are claims about the code, so cite something checkable: a symbol, `@doc/<path>` or `@task-<id>`, never a line number. `preference` is the user's own commitment, so cite who said it and when, and give it a `**Why:**` line; the reason is the only thing that lets a rule apply to a case it does not name.\n")
	sb.WriteString("- Read with `memory({ action: \"list\" })` at session start. Write with `memory({ action: \"add\" })` when work settles something durable: a write nothing conflicts with is live at once, while one matching an existing Memory comes back for review instead of overwriting it.\n\n")
